...
class F57k12c extends Model
{
    ...
    public function getCStatuslist()
    {
        return ['新建' => __('新建'), '处理中' => __('处理中'), '已回复' => __('已回复'), '已评价' => __('已评价')];
    }
    ...
    public function getCStatusTextAttr($value, $data)
    {
        $value = $value ? $value : (isset($data['CStatus']) ? $data['CStatus'] : '');
        $list = $this->getCStatuslist();
